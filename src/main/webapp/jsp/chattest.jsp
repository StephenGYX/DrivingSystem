...
<script>
	...
	function isEnter(Input, Hint, type, e) {
		...
		if (e.keyCode == 13) {
			...
			if (shiftDown == true) {
				...
				e.preventDefault();
				// createInfo(type, Input.value);//发送消息
				//
				// submityouText(Input.value);//后端回复
				toget();
				Input.value = null;
				Input.focus();
	...
	$('.chat-input-button').click(function () {/*消息框发送*/
		...
		if (chatInput.value != '') {
			// createInfo('you', chatInput.value);//发送消息
			//
			// submityouText(chatInput.value);//后端回复
	...
	function toget(){
		...
		$.ajax({
			...
			data:JSON.stringify({
				...
				"userInfo": {
					"apiKey": "59c0c58665234576abeb1e80b0f4b2ee",
					"userId": "test"//用户id
				}
			}),
			dataType: "text",
