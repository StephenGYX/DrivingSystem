			...
			if(layEvent === 'test1'){
					...
					if(method == "dialog"){
						layer.open({
							...
							btn1: function(index, layero){
								...
								console.log(s);
								$.ajax({
									url:"<%=path%>"+"/backConsumer/kaoShiArrange?cid="+cid+"&pid="+row_data.cpritiseid+"",
									type:'post',
									data: formData,
							...
							//要弹出的窗口的路径
							content: "<%=path%>"+"/backConsumer/kaoShiDialog?test="+layEvent+""  //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
				...
				layer.confirm("您确定要撤回用户 ："+row_data.cname+" 的科目一考试安排吗?", function (index) {
					...
					$(function() {
						$.ajax({
							method : "POST",
							url : "<%=path%>"+"/backConsumer/kaoShiRevoke?do=Revoke1&cid="+cid+"&pid="+row_data.cpritiseid+"",
							dataType : "text",
							success : function(msg) {
								if(msg>0){
				...
				layer.confirm("您确定要撤回用户 ："+row_data.cname+" 的科目二考试安排吗?", function (index) {
					...
					$(function() {
						$.ajax({
							method : "POST",
							url : "<%=path%>"+"/backConsumer/kaoShiRevoke?do=Revoke2&cid="+cid+"&pid="+row_data.cpritiseid+"",
							dataType : "text",
							success : function(msg) {
								if(msg>0){
				...
				layer.confirm("您确定要撤回用户 ："+row_data.cname+" 的科目三考试安排吗?", function (index) {
					...
					$(function() {
						$.ajax({
							method : "POST",
							url : "<%=path%>"+"/backConsumer/kaoShiRevoke?do=Revoke3&cid="+cid+"&pid="+row_data.cpritiseid+"",
							dataType : "text",
							success : function(msg) {
								if(msg>0){
				...
				layer.confirm("您确定要撤回用户 ："+row_data.cname+" 的科目四考试安排吗?", function (index) {
					...
					$(function() {
						$.ajax({
							method : "POST",
							url : "<%=path%>"+"/backConsumer/kaoShiRevoke?do=Revoke4&cid="+cid+"&pid="+row_data.cpritiseid+"",
							dataType : "text",
							success : function(msg) {
								if(msg>0){
