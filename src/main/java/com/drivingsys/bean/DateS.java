...
public class DateS
{
	...
	// 获取本周的开始时间
	@SuppressWarnings("unused")
	public static Date getBeginDayOfWeekByDay(Date date) {
		if (date == null) {
			return null;
		}
		Calendar cal = Calendar.getInstance();
	...
	// 获取本周的结束时间
	public static Date getEndDayOfWeek(Date first) {
		Calendar cal = Calendar.getInstance();
		cal.setTime(first);
	...
	// 获取前几月的结束时间
	public static Date getEndDayOfNumMonth(int i) {
		Calendar calendar = Calendar.getInstance();
		calendar.set(getNowYear(), getNowMonth() - i, 1);
		int day = calendar.getActualMaximum(5);
		calendar.set(getNowYear(), getNowMonth() - i, day);
		return getDayEndTime(calendar.getTime());
	}
	...
	// 获取上几个月的开始时间
	public static Date getBeginDayOfLastNumMonth(int num) {
		Calendar calendar = Calendar.getInstance();
